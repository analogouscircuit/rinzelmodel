...
def run_matlab_auditory_periphery(
    ...
    tauCa = np.array( [190e-6, 80e-6, 50e-6] )
    anf_pad = np.array([max(anf_num) for k in range(3)])
    ...
    matlab.workspace.MAP1_14(
        ...
        params_name,
        'spikes',
        ['IHCpreSynapseParams.tauCa=[ {} ];'.format( tauCa[np.where(anf_pad>0)] ),
         'AN_IHCsynapseParams.numFibers={};'.format(max(anf_pad)),
    ...
    cf_col = np.tile(np.repeat(cf_raw,max(anf_pad)), 3)
    type_col = np.repeat(['lsr', 'msr', 'hsr'], len(cf_raw)*max(anf_pad))

    # types = ['lsr', 'msr', 'hsr']
    # type_con = []
    # for k, t in enumerate(types):
    #     type_con.append( np.repeat( t, len(cf_raw)*anf_num[2-k] ) )
    # type_col = np.concatenate(type_con)
    # cf_col = np.concatenate( [ np.repeat(cf_raw, num) for num in anf_num ])
    # print(all_trains)
    # print("length anf_raw: ", len(anf_raw))
    # print("length type_col: ", len(type_col))
    # print("length cf_col: ", len(cf_col))
